...
Streamlit 화면으로 키워드, 실행 시간, 수신 이메일, 블로그 초안 설정을 바꿀 수 있습니다.
...
뉴스 중 하나를 골라 블로그 초안까지 만들고 싶으면 `.env`에 아래 값을 추가하세요.
...
`BLOG_PICK_INDEX=1`은 뉴스 목록의 첫 번째 뉴스를 고른다는 뜻입니다. `2`로 바꾸면 두 번째 뉴스를 고릅니다. 초안은 `blog_drafts` 폴더에 `.md` 파일로 저장되고, 메일에도 같이 들어갑니다.
